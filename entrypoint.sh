...
if [[ ${CONFIG_ENABLE} == "true" ]]; then
    ...
    ./config.sh REVIEWDOG_GIT_TOKEN REVIEWDOG_DIR REVIEWDOG_LVL REVIEWDOG_REPORTER REVIEWDOG_FILTER REVIEWDOG_FAIL REVIEWDOG_TFSEC_VERSION TRIVY_SCANTYPE TRIVY_SCANREF TRIVY_IGNORE TRIVY_SEVERITY
else
